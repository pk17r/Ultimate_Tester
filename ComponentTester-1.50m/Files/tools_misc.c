  ...
  while (Run)
  {
    ...
    if (Run)                       /* check opto coupler */
    {
      ...
      if (Test == DETECTED_BJT)         /* BJT type */
      {
        ...
        CTR /= U4;                      /* If = U/R in �A */
        U3 = (uint16_t)CTR;             /* If in �A */

        /* calculate BJT's Ie */
        /* Ie = I_total - If = (U2 / RiL) - If */
        CTR = (uint32_t)U2;             /* U2 (mV) */
        CTR *= 10000;                   /* scale to 0.0001 mV */
        CTR /= NV.RiL;                  /* /RiL in 0.1 Ohms -> I_total (�A) */ 
        CTR -= U3;                      /* Ie = I_total - If (�A) */
      ...
      if (Test == DETECTED_BJT)    /* BJT type */
      {
        ...
        if (U1 > 4000)        /* allow a leakage current of 1.5mA */
        {
          ...
          if (Run <= 250)          /* no overrun */
          {
            ...
            U1 /= MCU_CYCLES_PER_US;         /* delay (0.1 �s) */
          }
          ...
          if (Run <= 250)          /* no overrun */
          {
            ...
            U2 /= MCU_CYCLES_PER_US;         /* delay (0.1 �s) */
          }
    ...
    if (Flag & CHANGED_MODE)       /* mode has changed */
    {
      ...
      switch (Mode)                /* based on mode */
      {
        ...
        case MODE_LOW:             /* charge cap with low current (Rh) */
          /* max. charge current I = 5V/Rh = 10.6�A */
          Display_EEString_Space(CapCharge_str);
    ...
    if (Mode != MODE_NONE)
    {
      ...
      switch (Mode)                /* based on mode */
      {
        case MODE_HIGH:            /* charge cap with high current (Rl) */
          ...
          Value *= 100000;                   /* scale to 0.01 �V */
          Value /= ((R_LOW * 10) + NV.RiL);  /* 0.01 �V / 0.1 Ohms = 0.1 �A */
        ...
        case MODE_LOW:             /* charge cap with low current (Rh) */
          ...
          if (U1 > CAP_DISCHARGED)      /* minimum exceeded */
          {
            ...
            Value = U1;                        /* in mV */
            Value *= 10000;                    /* scale to 0.1 �V */
            Value /= (R_HIGH / 1000);          /* 0.1 �V / kOhms = 0.1 nA */
...
void PhotodiodeCheck(void)
{
  ...
  while (Flag > 0)
  {
    ...
    /* calculate I_P (= U / R) */
    I = U * 100000;                /* scale voltage to 0.01 �V */
    I /= R;                        /* / R (in 0.1 Ohms) -> I in 0.1 �A */ 
