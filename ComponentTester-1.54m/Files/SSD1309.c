/* ************************************************************************
 *
 *   driver functions for SSD1309 compatible OLED grafic displays
...
#include "SSD1309.h"           /* SSD1309 specifics */
...
void LCD_Init(void)
{
  ...
  LCD_Cmd(CMD_DISPLAY | FLAG_DISPLAY_OFF);     //--turn off oled panel

  LCD_Cmd(CMD_DISP_CLOCK);    //--set display clock divide ratio/oscillator frequency
  LCD_Cmd(FLAG_FREQ_9);       //--set divide ratio

  LCD_Cmd(CMD_MULTIPLEX_RATIO);   //--set multiplex ratio(1 to 64)
  LCD_Cmd(0x3f);                //--1/64 duty

  LCD_Cmd(CMD_DISP_OFFSET);     //-set display offset
  LCD_Cmd(0x00);                  //-not offset

  LCD_Cmd(CMD_START_LINE);      //--set start line address

  LCD_Cmd(CMD_DISP_MODE | FLAG_DISP_NORMAL);       //--set normal display

  LCD_Cmd(CMD_PIXEL_MODE | FLAG_PIXEL_RAM);      //Disable Entire Display On

  LCD_Cmd(CMD_SEGMENT_MAP | FLAG_SEG_131);    //--set segment re-map 128 to 0

  LCD_Cmd(CMD_COM_SCAN_DIR | FLAG_COM_63);    //--Set COM Output Scan Direction 64 to 0
  ...
  #else
  LCD_Cmd(CMD_COM_CONFIG_SET);      //--set com pins hardware configuration
  LCD_Cmd(FLAG_COM_ALT);
  #endif
  ...
  LCD_Cmd(CMD_DESELECT_LEVEL);      //--set vcomh
  LCD_Cmd(0x34);
